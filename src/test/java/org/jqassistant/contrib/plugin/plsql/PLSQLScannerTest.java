...
//        assertThat(plsqlc.size(), equalTo(2));
    ...
    public void scanSQLFileFunc() {
        store.beginTransaction();
        // Scan the test PLSQL file located as resource in the classpath
        File testFile = new File(getClassesDirectory(PLSQLScannerTest.class), "/test_function.sql");
        ...
        public void scanSQLFileTrig() {
            store.beginTransaction();
            // Scan the test PLSQL file located as resource in the classpath
            File testFile = new File(getClassesDirectory(PLSQLScannerTest.class), "/test_trigger.sql");
        ...
        public void scanSQLFileCursor() {
            store.beginTransaction();
            // Scan the test PLSQL file located as resource in the classpath
            File testFile = new File(getClassesDirectory(PLSQLScannerTest.class), "/test_cursor.sql");
